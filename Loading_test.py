...
def process_stock_data(data):
    """
    將包含股票數據的 list 轉換為結構化的字典格式。

    參數:
        data (list): 包含股票數據的列表，格式為
                     [日期時間, 開盤價, 最高價, 最低價, 收盤價, 成交量]

    返回:
        dict: 格式化後的數據字典，包含日期時間、開盤價、最高價、最低價、收盤價和成交量。
    """
...
# 處理多行數據並存入 DataFrame
# processed_data_list = [process_stock_data(row) for row in EventOSQ.KlineData]
...
# %%
StockNo ="CME,MNQ2412"
nCode = skOSQ.SKOSQuoteLib_RequestKLineByDate(StockNo,sKLineType=0,bstrStartDate="20241110",
                                              bstrEndDate="20241113",
                                              sMinuteNumber=1)
print(skC.SKCenterLib_GetReturnCodeMessage(nCode=nCode))
